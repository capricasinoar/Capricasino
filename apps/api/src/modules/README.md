...
| `realtime` | 6 ✅ | WS Gateway (Socket.IO) con auth JWT en handshake, saldo en vivo por evento `balance.changed`, rooms `user:{id}`, resync REST al reconectar. Pendiente: Redis adapter multi-instancia (S escalado) |
